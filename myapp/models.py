...
class PhoneNumberField(models.CharField):
    """Custom field for validating phone numbers in the format +92-XXXXXXXXX-1."""
    
    def __init__(self, *args, **kwargs):
        kwargs['max_length'] = 15  # Set a max length constraint
...
class Department(models.Model):
    name = models.CharField(
        max_length=100,
        validators=[MaxLengthValidator(100, message= 'Length is greatter than 100 letters')],  # Enforces max length explicitly
        blank=False
    )
    head = models.ForeignKey(
        ...
        blank=True  # Allows leaving this field blank
    )
...
class Teacher(models.Model):
    ...
    email = models.EmailField(
        unique=True,
        validators=[MaxLengthValidator(254)],  # Standard max length for emails
        blank=False
    )
...
class Classroom(models.Model):
    ...
    capacity = models.PositiveIntegerField(
        validators=[MinValueValidator(1)]  # Capacity must be at least 1
    )
    department = models.ForeignKey(
        Department,
        on_delete=models.SET_NULL,
